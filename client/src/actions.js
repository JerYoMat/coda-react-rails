...
export const LOAD_STOCK_DATA_BEGIN = "LOAD_STOCK_DATA_BEGIN";
export const LOAD_STOCK_DATA_SUCCESS = "LOAD_STOCK_DATA_SUCCESS";
export const LOAD_STOCK_DATA_ERROR = "LOAD_STOCK_DATA_ERROR";
...
export const loadStockData = id => {
  const companyId = id;
  return dispatch => {
    dispatch({ type: LOAD_STOCK_DATA_BEGIN });
    getStockPriceData(id)
      .then(response => {
        const rData = response["Weekly Adjusted Time Series"];
        const data = Object.keys(rData).map(week => {
          return [
            Date.parse(week),
            Number(Number(rData[week]["5. adjusted close"]).toFixed(2))
          ];
        });
        const payload = data.sort((date1, date2) => {
      ...
      .catch(error => {
        dispatch({ type: LOAD_STOCK_DATA_ERROR, error });
      });
  };
};
